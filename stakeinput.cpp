...
// The zCBC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zCBC is a hash of the serial
...
{
    //Create an output returning the zCBC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZCBCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zCBC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zCBC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZCBCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zCBC output", __func__);
...
{
    CzCBCTracker* zcbcTracker = pwallet->zcbcTracker.get();
    CMintMeta meta;
    if (!zcbcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zcbcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a CBC stake is the outpoint
