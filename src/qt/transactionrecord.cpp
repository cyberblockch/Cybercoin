...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZCBCModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zcbcTracker->HasMintTx(hash))) {
            //zCBC stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZCBC;
...
bool IsZCBCType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZCBC:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZCBC || type == TransactionRecord::MNReward) {
