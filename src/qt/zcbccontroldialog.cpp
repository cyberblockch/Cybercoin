...
#include "ui_zcbccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zcbcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zcbcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zCBC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zCBC seed used to mint this zCBC is not the same as currently hold in the wallet";
            else
