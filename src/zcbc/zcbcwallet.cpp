...
CzCBCWallet::CzCBCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcbc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCBCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCBCSeed_deprecated()) {
                    LogPrintf("%s: Updated zCBC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcbc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcbc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCBCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCBCCount(nCountLastUsed);
    else if (!walletdb.ReadZCBCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCBCWallet::Lock()
{
    seedMaster = 0;
}

void CzCBCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCBCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCBC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCBCWallet::LoadMintPoolFromDB()
{
...
void CzCBCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCBCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCBCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcbcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCBCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCBC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcbcTracker which also adds to database
    pwalletMain->zcbcTracker->Add(dMint, true);
...
void CzCBCWallet::SeedToZCBC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCBCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCBCWallet::GenerateDeterministicZCBC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCBCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCBCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCBCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
