...
CzCBCTracker::CzCBCTracker(std::string strWalletFile)
{
...
CzCBCTracker::~CzCBCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCBCTracker::Init()
{
...
bool CzCBCTracker::Archive(CMintMeta& meta)
{
...
bool CzCBCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCBCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCBCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCBCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzCBCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzCBCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzCBCTracker::GetSerialHashes()
{
...
CAmount CzCBCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCBCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCBCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCBCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCBCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCBCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCBCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCBCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCBCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCBCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCBCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCBCWallet* zCBCWallet)
{
    bool iszCBCWalletInitialized = (NULL != zCBCWallet);
    CMintMeta meta;
    ...
    if (! iszCBCWalletInitialized)
        zCBCWallet = new CzCBCWallet(strWalletFile);
    meta.isSeedCorrect = zCBCWallet->CheckSeed(dMint);
    if (! iszCBCWalletInitialized)
        delete zCBCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCBCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCBCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCBCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCBCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCBCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCBCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCBCWallet* zCBCWallet = new CzCBCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zCBCWallet);
        }
        delete zCBCWallet;
        LogPrint("zero", "%s: added %d dzcbc from DB\n", __func__, listDeterministicDB.size());
    }
