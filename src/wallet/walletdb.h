...
public:
    ...
    bool WriteZCBCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCBCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCBCSeed_deprecated(uint256& seed);
    bool EraseZCBCSeed();
    bool EraseZCBCSeed_deprecated();

    bool WriteZCBCCount(const uint32_t& nCount);
    bool ReadZCBCCount(uint32_t& nCount);
