...
- [Development guidelines](#development-guidelines)
    - [General Cybercoin Core](#general-cybercoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Cybercoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play CBCs (tCBC)" on the test network, if you
...
Cybercoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/cybercoin-project/cybercoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_cybercoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_cybercoin --log_level=test_suite
$ valgrind -v --leak-check=full src/cybercoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_cybercoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates CBCs (if wallet is enabled).
...
However, in open source software such as Cybercoin Core, where everyone uses
...
pay attention to for reviewers of Cybercoin Core code.

General Cybercoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Cybercoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Cybercoin Core so that they can be integrated
...
file descriptors it will cause problems with Cybercoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Cybercoin-Project/Cybercoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `cybercoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `cybercoin-cli` and the GUI debug console use this table to determine how to
